...
## Processing Steps
Shapefile fields are limited to 10 characters, so the RDH renamed all fields, including for the csv files, that exceeded 10 characters to prepare for shapefile processing. 
...
The original dataset from Dr. Klarner contained all US states in one file. The RDH processed all the the data, split the file by state and exported data for each state as a separate csv using Python and the Pandas library.
...
++Race codes below:
NOTE: Multiple codes can be given for one person. Every Cuban also coded "h" so the code "c" always appears as "ch"
