...
def load_cvap_block_groups(cvap_blockgr_path: Path, state_fips: str) -> pd.DataFrame:
    ...
    def col(name: str) -> pd.Series:
        if name in wide.columns:
            return pd.to_numeric(wide[name], errors="coerce").fillna(0)
...
def _series_int64(x: pd.Series) -> pd.Series:
    return pd.to_numeric(x, errors="coerce").fillna(0).round().astype("int64")
...
def build_processed_inputs(
    ...
    bgs = gpd.GeoDataFrame(
        blocks2[["geoid_bg", "geometry"]].dissolve(by="geoid_bg", as_index=False),
        crs=blocks2.crs,
    )
    ...
    weights_raw = pd.to_numeric(blocks2[weight_col], errors="coerce").fillna(0)
    denom = blocks_to_bg.map(weights_raw.groupby(blocks_to_bg, observed=True).sum())
    weights = (weights_raw / denom.replace(0, np.nan)).fillna(0)
    ...
    cvap_by_vtd = (
        block_cvap.groupby(blocks_to_vtd, observed=True)
        .sum()
        .reindex(vtds.index, fill_value=0)
        .apply(lambda s: np.rint(s).astype("int64"))
    )
    ...
    for col in ["total_hisp", "total_nh_white", "total_nh_black", "total_nh_asian", "total_nh_native"]:
    ...
    for col in ["vap_hisp", "vap_nh_white", "vap_nh_black", "vap_nh_asian", "vap_nh_native"]:
        if col not in geo.columns:
            geo[col] = 0
    ...
    returns_vtd["votes_rep"] = (returns_vtd["votes_total"] - returns_vtd["votes_dem"]).clip(lower=0)
    returns_vtd["votes_other"] = 0
