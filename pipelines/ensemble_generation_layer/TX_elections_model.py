...
# GerryChain now expects seeding through Python's standard random module.
# Use a fixed default seed for reproducibility, while still allowing override
# through an environment variable when desired.
RANDOM_SEED = int(os.environ.get("GERRYCHAIN_RANDOM_SEED", "20260324"))
...
# Compatibility patch for legacy code paths that incorrectly use .at with a
# boolean row mask. In modern pandas, .at only accepts scalar row/column labels.
# When a boolean mask is passed, transparently route the assignment through .loc.
_original_at_setitem = _AtIndexer.__setitem__

def _patched_at_setitem(self, key, value):
    if isinstance(key, tuple) and len(key) == 2:
        row_key, col_key = key
        if isinstance(row_key, (pd.Series, list, tuple, np.ndarray)):
            self.obj.loc[row_key, col_key] = value
            return
    return _original_at_setitem(self, key, value)
...
plot_path = 'data/input/TX_VTDs/TX_VTDs.shp'  #for shapefile
...
# Preferred projected CRS for Texas-wide area/centroid operations.
TEXAS_PROJECTED_CRS = "EPSG:3083"

def _clean_and_project_geometries(gdf):
    """Repair common geometry issues and project to a planar CRS for area/centroid work."""
    gdf = gdf.copy()
    # Normalize invalid/self-touching geometries first.
    if hasattr(gdf.geometry, "make_valid"):
        gdf["geometry"] = gdf.geometry.make_valid()
    else:
        gdf["geometry"] = gdf.geometry.buffer(0)

    # A second zero-width buffer often resolves tiny topology artifacts/slivers.
    gdf["geometry"] = gdf.geometry.buffer(0)

    if gdf.crs is None:
        raise ValueError("Input shapefile has no CRS; cannot safely compute areas/centroids.")

    if str(gdf.crs) != TEXAS_PROJECTED_CRS:
        gdf = gdf.to_crs(TEXAS_PROJECTED_CRS)

    return gdf

#read files###################################################################
elec_data = pd.read_csv("data/input/TX_elections.csv")
TX_columns = list(pd.read_csv("data/input/TX_columns.csv")["Columns"])
dropped_elecs = pd.read_csv("data/input/dropped_elecs.csv")["Dropped Elections"]
recency_weights = pd.read_csv("data/input/recency_weights.csv")
min_cand_weights = pd.read_csv("data/input/ingroup_weight.csv")
cand_race_table = pd.read_csv("data/input/Candidate_Race_Party.csv")
EI_statewide = pd.read_csv("data/input/statewide_rxc_EI_preferences.csv")
prec_ei_df = pd.read_csv("data/input/prec_count_quants.csv", dtype = {'CNTYVTD':'str'})
mean_prec_counts = pd.read_csv("data/input/mean_prec_vote_counts.csv", dtype = {'CNTYVTD':'str'})
logit_params = pd.read_csv('data/input/TX_logit_params.csv')
...
# Repair geometry and project before any graph, area, or centroid calculations.
state_gdf = _clean_and_project_geometries(state_gdf)
...
centroids_geom = state_gdf.geometry.centroid
state_gdf[C_X] = centroids_geom.x
state_gdf[C_Y] = centroids_geom.y
for node in graph.nodes():
    graph.nodes[node]["C_X"] = state_gdf.at[node, C_X]
    graph.nodes[node]["C_Y"] = state_gdf.at[node, C_Y]
...
store_plans["GEOID"] = [state_gdf[geo_id].iloc[i] for i in store_plans["Index"]]
