    ...
    for district in dist_changes:
        ...
        for elec_set in elec_sets:
            ...
            if 'Runoff' in elec_set_dict[elec_set].keys():
                ...
                hisp_pref_cand_dist = max(HCVAP_support_elec.items(), key=operator.itemgetter(1))[0]
                hisp_pref_cands_runoffs_dist.loc[
                ...
                black_pref_cand_dist = max(BCVAP_support_elec.items(), key=operator.itemgetter(1))[0]
                black_pref_cands_runoffs_dist.loc[
...
def compute_final_dist(map_winners, black_pref_cands_df, black_pref_cands_runoffs, \
    ...
    # Strip the office suffix from CANDIDATE_RACE_PARTY's "Candidates" keys so
    # the party lookup uses the same Name+Party_Stage form that the rest of the
    # pipeline (parquet columns, EI table, gerrychain Elections) uses.
    cand_party_dict = {
        "_".join(full_key.split("_")[:2]): party
        for full_key, party in cand_race_table.set_index("Candidates")["Party"].items()
    }
...
def cand_pref_outcome_sum(prec_draws_outcomes, dist_prec_indices, bases, outcomes):
    ...
    for outcome in outcomes:
        ...
        # Candidate column basenames look like "<demog>.<election>_<cand>"
        # where the candidate is always the last 2 underscored tokens (e.g.
        # "ClarkR_24P"). Extract just that so the resulting dict can be looked
        # up against cand_race_dict / cand_party_dict downstream.
        def _cand(b):
            return "_".join(b.split('.')[1].split('_')[-2:])

        prefs = {_cand(x): 0.0 for x in outcomes[outcome]}
        prefs_counts = dict(zip(unique, counts))
        prefs.update({_cand(outcomes[outcome][key]): prefs_counts[key] / len(summed_outcome[0])
                      for key in prefs_counts.keys()})
